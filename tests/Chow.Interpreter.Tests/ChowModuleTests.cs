...
namespace Chow.Interpreter.Tests
{
    ...
    class ChowModuleTests
    {
        ...
        public void Constructor_BuiltInFunction_ExistsAndDoesNotThrow(string builtInName)
        {
            Assert.That(() => NewModule()[builtInName], Throws.Nothing);
        }

        [Test]
        public void Constructor_NullName_UsesDefaultName()
        {
            var module = new ChowModule(name:null);
            Assert.That(module.Name, Is.EqualTo(nameof(ChowModule)));
        }
