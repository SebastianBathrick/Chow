<!-- Source: Crafting Interpreters, Chapter 4: Scanning -->
...
(print (eval (read))) Working outwards from the most nested call, you Read a line of input, Evaluate it, Print the result, then Loop and do it all over again.
...
.1234 1234. We could easily support the former, but I left it out to keep things simple. The latter gets weird if we ever want to allow methods on numbers like 123.sqrt().
...
first -second Is this first - second—an infix subtraction—or two expression statements, one for first and one to negate second?
...
Next Chapter: “Representing Code” →Handcrafted by Robert Nystrom — © 2015 – 2021
